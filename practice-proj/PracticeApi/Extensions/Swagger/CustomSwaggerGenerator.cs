...
{
    internal class CustomSwaggerGenerator : ISwaggerProvider
    {
        private readonly IApiDescriptionGroupCollectionProvider _apiDescriptionsProvider;
        private readonly ISchemaGenerator _schemaGenerator;
        private readonly SwaggerGeneratorOptions _options;

        public CustomSwaggerGenerator(
        ...
        private OpenApiPaths GeneratePaths(IEnumerable<ApiDescription> apiDescriptions, SchemaRepository schemaRepository)
        {
            var apiDescriptionsByPath = apiDescriptions
                ...
                .GroupBy(apiDesc => apiDesc.GetGroupKey());
        ...
        private IList<OpenApiParameter> GenerateParameters(ApiDescription apiDescription, SchemaRepository schemaRepository)
        {
            var applicableApiParameters = apiDescription.ParameterDescriptions
                .Where(apiParam => (!apiParam.IsFromBody() && !apiParam.IsFromForm())
            ...
            return applicableApiParameters
                .Select(apiParam => GenerateParameter(apiParam, schemaRepository))
                .ToList();
        }
            ...
            SchemaRepository schemaRepository)
        {
            ...
            if (schema.Reference == null)
            {
                ...
                {
                    var attribute = parameterInfo.GetCustomAttribute<RequiredFromQueryAttribute>();
                    if (parameterInfo.HasDefaultValue || attribute?.Value != null)
                    {
                        schema.Default = OpenApiAnyFactory.CreateFor(schema,
                            parameterInfo.HasDefaultValue ? parameterInfo.DefaultValue : attribute?.Value);
                    }
                }
        ...
        {
            var contentTypes = InferRequestContentTypes(apiDescription).ToList();
            contentTypes = contentTypes.Count > 0 ? contentTypes : new List<string> { "multipart/form-data" };
        ...
        private OpenApiMediaType CreateResponseMediaType(ModelMetadata modelMetadata, SchemaRepository schemaRepository)
        {
            return new OpenApiMediaType
            {
                Schema = _schemaGenerator.GenerateSchema(modelMetadata.ModelType, schemaRepository)
            };
        }
